...
def init_scheduler_module(app, socketio, scheduler):
    ...
    def schedule_streaming_api():
        try:
            data = request.json
            logger.info(f"Menerima data penjadwalan: {data}")
            ...
            for i, sched in enumerate(s_data.get('scheduled_sessions', [])):
                if sched.get('session_name_original') == session_name_original:
                    logger.info(f"Menemukan jadwal yang sudah ada dengan nama sesi asli '{session_name_original}', akan menggantinya.")
                    ...
                    try:
                        ...
                        logger.info(f"Job scheduler lama untuk '{session_name_original}' berhasil dihapus.")
                    except Exception as e_remove_old_job:
                        logger.info(f"Tidak ada job scheduler lama untuk '{session_name_original}' atau error saat menghapus: {e_remove_old_job}")
                    idx_to_remove = i
                    break
            if idx_to_remove != -1:
            ...
            if recurrence_type == 'daily':
                ...
                logger.info(f"Jadwal harian START '{aps_start_job_id}' untuk '{session_name_original}' ditambahkan: {start_time_of_day}")
                ...
                logger.info(f"Jadwal harian STOP '{aps_stop_job_id}' untuk '{session_name_original}' ditambahkan: {stop_time_of_day}")
                ...
                logger.info(f"Jadwal sekali jalan START '{aps_start_job_id}' untuk '{session_name_original}' ditambahkan pada {start_dt}")
                ...
                if not is_manual_stop:
                    ...
                    logger.info(f"Jadwal sekali jalan STOP '{aps_stop_job_id}' untuk '{session_name_original}' ditambahkan pada {stop_dt}")
            ...
            logger.error(f"Input tidak valid untuk penjadwalan: {str(e)}", exc_info=True)
            ...
            logger.error(f"Error server saat menjadwalkan sesi '{session_name_err_sched}'", exc_info=True)
...
def start_scheduled_streaming(platform, stream_key, video_file, session_name_original, 
    ...
    logger.info(f"Mulai stream terjadwal: '{session_name_original}', Tipe: {recurrence_type}, Durasi One-Time: {one_time_duration_minutes} menit, Jadwal Harian: {daily_start_time_str}-{daily_stop_time_str}")
    
    video_path = os.path.abspath(os.path.join(VIDEOS_DIR, video_file))
    if not os.path.isfile(video_path):
        logger.error(f"Video {video_file} tidak ada untuk jadwal '{session_name_original}'. Jadwal mungkin perlu dibatalkan.")
        return
    ...
    try:
        ...
        logger.info(f"Service {service_name_systemd} untuk jadwal '{session_name_original}' dimulai.")
        ...
        from . import socketio_instance
        socketio_instance.emit('sessions_update', get_active_sessions_data())
        socketio_instance.emit('schedules_update', get_schedules_list_data())
        logger.info(f"Sesi terjadwal '{session_name_original}' (Tipe: {recurrence_type}) dimulai, update dikirim.")

    except Exception as e:
        logger.error(f"Error start_scheduled_streaming untuk '{session_name_original}': {e}", exc_info=True)

def stop_scheduled_streaming(session_name_original_or_active_id):
    """Stop a scheduled streaming session"""
    logger.info(f"Menghentikan stream (terjadwal/aktif): '{session_name_original_or_active_id}'")
...
# Set socketio instance for use in scheduled functions
socketio_instance = None

def set_socketio_instance(socketio):
    """Set the socketio instance for use in scheduled functions"""
