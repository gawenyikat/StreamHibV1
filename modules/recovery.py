        ...
        if json_changed: 
            ...
            try:
                ...
                if socketio_instance:
                    ...
                    socketio_instance.emit('inactive_sessions_update', {"inactive_sessions": get_inactive_sessions_data()})
            except:
                pass
    except Exception as e: 
    ...
    try:
        ...
        for sched_item in scheduled_sessions_copy:
            ...
            try:
                ...
                if scheduler_instance:
                    if recurrence == 'daily':
                        try: scheduler_instance.remove_job(f"daily-start-{sanitized_id}")
                        except: pass
                        try: scheduler_instance.remove_job(f"daily-stop-{sanitized_id}")
                        except: pass
                    elif recurrence == 'one_time':
                        try: scheduler_instance.remove_job(schedule_def_id)
                        except: pass
                        if not sched_item.get('is_manual_stop', sched_item.get('duration_minutes', 0) == 0):
                            try: scheduler_instance.remove_job(f"onetime-stop-{sanitized_id}")
                            except: pass
            except:
                pass
        ...
        try:
            ...
            if socketio_instance:
                ...
                from .videos import get_videos_list
                socketio_instance.emit('videos_update', get_videos_list())
                socketio_instance.emit('trial_reset_notification', {
                    'message': 'Aplikasi telah direset karena mode trial. Semua sesi dan video telah dihapus.'
                })
                socketio_instance.emit('trial_status_update', {
...
# Set scheduler instance for use in trial reset
scheduler_instance = None

def set_scheduler_instance(scheduler):
    """Set the scheduler instance for use in recovery functions"""
