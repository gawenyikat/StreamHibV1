...
"""
    try:
        ...
        logger.info(f"Service file created: {service_name} (from original: '{session_name_original}')")
        return service_name, sanitized_service_part
    except Exception as e:
        logger.error(f"Error creating service file {service_name} (from original: '{session_name_original}'): {e}")
        raise
...
def stop_streaming_session(session_id_to_stop):
    ...
    try:
        ...
        if active_session_data and 'sanitized_service_id' in active_session_data:
            ...
            logger.warning(f"Menggunakan fallback sanitized_service_id '{sanitized_service_id_for_stop}' untuk menghentikan sesi '{session_id_to_stop}'.")
        ...
        try:
            ...
            if os.path.exists(service_path): 
             ...
             logger.warning(f"Peringatan saat menghentikan/menghapus service {service_name_systemd}: {e_service_stop}")
        ...
        logger.info(f"Stopped streaming session: {session_id_to_stop}")
        return True
