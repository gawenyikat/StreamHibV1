...
# Set socketio and scheduler instances for cross-module communication
from modules.scheduler import set_socketio_instance
from modules.recovery import set_scheduler_instance
...
def recover_schedules():
    ...
    try:
        ...
        logger.info("Memulai pemulihan jadwal...")
        for sched_def in s_data.get('scheduled_sessions', []):
            try:
                ...
                if not all([session_name_original, sanitized_service_id, platform, stream_key, video_file, schedule_definition_id]):
                    logger.warning(f"Recover: Skip jadwal '{session_name_original}' karena field dasar kurang.")
                    continue
                ...
                if recurrence == 'daily':
                    ...
                    if not start_time_str or not stop_time_str:
                        logger.warning(f"Recover: Skip jadwal harian '{session_name_original}' karena field waktu harian kurang.")
                        continue
                    ...
                    logger.info(f"Recovered daily start job '{aps_start_job_id}' for '{session_name_original}' at {start_time_str}")
                    ...
                    logger.info(f"Recovered daily stop job '{aps_stop_job_id}' for '{session_name_original}' at {stop_time_str}")
                    ...
                    if not start_time_iso or duration_minutes is None:
                        logger.warning(f"Recover: Skip jadwal one-time '{session_name_original}' karena field waktu/durasi kurang.")
                        continue
                    ...
                    if start_dt > now_jkt:
                        ...
                        logger.info(f"Recovered one-time start job '{aps_start_job_id}' for '{session_name_original}' at {start_dt}")
                        ...
                        if not is_manual:
                            ...
                            if stop_dt > now_jkt:
                                ...
                                logger.info(f"Recovered one-time stop job '{aps_stop_job_id}' for '{session_name_original}' at {stop_dt}")
                        valid_schedules_in_json.append(sched_def)
                    else:
                        logger.info(f"Recover: Skip jadwal one-time '{session_name_original}' karena waktu sudah lewat.")
                else:
                     logger.warning(f"Recover: Tipe recurrence '{recurrence}' tidak dikenal untuk '{session_name_original}'.")

            except Exception as e:
                logger.error(f"Gagal memulihkan jadwal '{sched_def.get('session_name_original', 'UNKNOWN')}': {e}", exc_info=True)
        ...
        if len(s_data.get('scheduled_sessions', [])) != len(valid_schedules_in_json):
            ...
            logger.info("File sessions.json diupdate dengan jadwal yang valid setelah pemulihan.")
        logger.info("Pemulihan jadwal selesai.")
        
    except Exception as e:
        logger.error(f"Error recovering schedules: {e}", exc_info=True)
